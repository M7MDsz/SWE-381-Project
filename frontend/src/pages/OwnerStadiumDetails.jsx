  ...
  const handleCreateSlot = async (e) => {
    ...
    try {
      await authFetch('/stadiums/slots', {
        method: 'POST',
        body: JSON.stringify({ ...slotForm, stadiumId })
      });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <Link to="/owner" className="btn btn-link ps-0">← Back to Dashboard</Link>
        <button className="btn btn-success" onClick={() => setShowModal(true)}>Add Slot</button>
      </div>

      <div className="card card-body shadow-sm mb-4">
        <h3 className="text-success mb-1">{stadium.name}</h3>
        <p className="mb-1"><strong>Location:</strong> {stadium.location}</p>
        <p className="mb-2"><strong>Description:</strong> {stadium.description}</p>
        {stadium.facilities && stadium.facilities.length > 0 && (
          <div>
            <strong>Facilities:</strong>
            <ul className="mb-0 mt-1">
              {stadium.facilities.map((facility) => <li key={facility}>{facility}</li>)}
            </ul>
          </div>
        )}
      </div>
      ...
      {slots.length === 0 && <p className="text-muted">No slots yet. Click “Add Slot” to create one.</p>}
          ...
          <div className="modal-panel card shadow-lg">
            <div className="card-body">
              ...
              <form onSubmit={handleCreateSlot}>
                ...
                <input type="time" className="form-control mb-3" name="endTime" value={slotForm.endTime} onChange={handleSlotChange} required />
                <button className="btn btn-success" type="submit">Save Slot</button>
              </form>
            </div>
          </div>
        </div>
